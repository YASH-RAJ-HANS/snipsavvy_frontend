            ...
            <div className='-mt-5 ml-5'>
                {Collections.map((collection, index) =>(
                    <div key= {index} className = 'h-14 w-14  m-auto ml-7 rounded-lg ' >
                        <Link href={`/workspace/${collection.collectionId}`}><button className='text-slate-400 flex text-sm hover:border-2'><span style={{ color: `${collection.color}` , marginRight:"10px", fontWeight:"100px"}}>•</span> {collection.name}</button></Link>
                    </div>
                ))}
            </div>
